...
##### 添加水平分割线	
			...
			<hr width="" size="" color="" align="" noshade> 											语法解释：
...
#####有序列表	
		      ...
		      <li>项目名称</li>
																																																																						...
																																																																						type属性值如下：																																																																																			1，a，A，i（小写罗马数字），I（大写罗马数字）
						    ...
						    loop:设置移动文字的循环次数：infinite:文字移动无限循环； 正整数n:移动n次数										
...
>表单包含两类元素：一是普通的页面元素：二是用于收集信息的特定页面元素，即表单控件（文本框，按钮，单选框，复选框，下拉列表，隐藏框和文件选择输入框都属于表单控件）。
				...
				* value属性为文本框的值，可以通过设置vaule属性的值来指定当表单首次被载入时显示								
